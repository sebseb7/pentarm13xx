...
void SSP_IOConfig( uint8_t portNum )
{
  if ( portNum == 0 )
  {
	...
	LPC_SYSCON->SYSAHBCLKCTRL |= (0x1<<11); // bit 11 enables clock for SSP

	LPC_SYSCON->SSP0CLKDIV = 0x01;			/* Divided by 1 (1..255) */
	...
	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<6); // bit 6 enables GPIO Clock
